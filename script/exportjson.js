...
function exportOptions() {
  ...
  grid.setWidget(3, 0, makeButton(app, grid, 'Export Active Sheet', 'exportJSONSheet'));
  grid.setWidget(3, 1, makeButton(app, grid, 'Export All Sheets', 'exportJSONAllSheets'));
  app.add(grid);
...
function exportAllSheets(e, format) {
...
function displayText_(text, format) {
  var app = UiApp.createApplication().setTitle('Exported ' + format);
  app.add(makeTextBox(app, 'text'));
  app.getElementById('text').setText(text);
  var ss = SpreadsheetApp.getActiveSpreadsheet(); 
  ss.show(app);
  return app; 
}
...
function getRowsData_(sheet, options) {
  if (sheet.getFrozenRows() == 0) {
    throw new Error("The topmost row needs to be frozen to determine headers.");
  }
